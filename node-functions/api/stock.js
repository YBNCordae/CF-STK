...
function markPriceRangeHits(items, priceRange) {
  return items.map((item) => ({
    ...item,
    in_price_range: priceRange ? isPriceInRange(item.close, priceRange) : null,
  }));
}
...
function isPriceInRange(close, priceRange) {
  if (!Number.isFinite(close)) return false;
  const closeScaled = toScaledInt(close, priceRange.scale);
  return closeScaled >= priceRange.low_scaled && closeScaled <= priceRange.high_scaled;
}
